...
test.describe('comparison Select visual parity', () => {
  test('React and Solid Select stay visually and behaviorally comparable in viewport', async ({ page }) => {
    ...
    expect(Math.abs(reactListGeometry.y - (reactTriggerGeometry.y + reactTriggerGeometry.height))).toBeLessThanOrEqual(24);
    expect(Math.abs(solidListGeometry.y - (solidTriggerGeometry.y + solidTriggerGeometry.height))).toBeLessThanOrEqual(24);
    ...
    await solidTrigger.click();
    const solidListBoxForSelect = page.locator('.comparison-spectrum-Select-listbox[role="listbox"]');
    ...
    await solidTrigger.click();
    const solidListBoxForOutside = page.locator('.comparison-spectrum-Select-listbox[role="listbox"]');
