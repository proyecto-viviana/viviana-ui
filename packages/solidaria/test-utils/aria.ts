...
export function getAriaRole(element: Element): string | null {
  ...
  const implicitRoles: Record<string, string> = {
    ...
    main: 'main',
    nav: 'navigation',
    section: element.hasAttribute('aria-label') || element.hasAttribute('aria-labelledby') ? 'region' : '',
    form: element.hasAttribute('aria-label') || element.hasAttribute('aria-labelledby') ? 'form' : '',
    table: 'table',
    thead: 'rowgroup',
    tbody: 'rowgroup',
    tfoot: 'rowgroup',
...
export function assertAccessible(element: Element): void {
  ...
  const interactiveRoles = ['button', 'link', 'checkbox', 'radio', 'textbox', 'combobox', 'listbox', 'menu', 'menuitem', 'slider', 'spinbutton', 'switch', 'tab'];
