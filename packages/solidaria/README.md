...
function Button(props) {
  ...
  return (
    <button {...buttonProps} ref={ref} class={isPressed() ? "pressed" : ""}>
      {props.children}
    </button>
  );
}
...
```tsx
<Button onPress={() => console.log("Pressed!")}>Works with click, touch, and keyboard</Button>
```
...
```tsx
import { createButton, type AriaButtonProps, type ButtonAria } from "@proyecto-viviana/solidaria";
```
