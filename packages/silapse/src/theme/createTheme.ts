/**
 * Reactive theme hook for SolidJS apps using Silapse.
 *
...
const STORAGE_KEY = 'silapse-theme'
...
export interface SilapseThemeResult {
...
/**
 * Create a reactive Silapse theme.
 *
 ...
 * const { scheme, isDark, toggleScheme } = createSilapseTheme()
 *
 * // Use a custom theme
 * const custom = generateTheme({ primary: '#7C3AED', accent: '#F59E0B' })
 * const { scheme, isDark, toggleScheme } = createSilapseTheme(custom)
 * ```
 */
export function createSilapseTheme(theme: GeneratedTheme = viviana): SilapseThemeResult {
  onMount(() => init(theme))
