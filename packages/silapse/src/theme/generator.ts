/**
 * Theme generator — derives a complete Silapse palette from 2 colors.
 *
...
function generateSemanticScale(
  hue: number,
  chroma: number,
  mode: 'dark' | 'light',
): SemanticScale {
  ...
  const base = oklchToHex({ l: isDark ? 0.70 : 0.55, c: chroma, h: hue })
  const dim = oklchToHex({ l: isDark ? 0.45 : 0.50, c: chroma * 0.7, h: hue })
  ...
  const lightness = isDark
    ? [0.95, 0.88, 0.78, 0.70, 0.60, 0.48, 0.38, 0.28, 0.18]
    : [0.95, 0.88, 0.78, 0.55, 0.60, 0.40, 0.45, 0.35, 0.20]
  const chromaMul = [0.2, 0.35, 0.55, 0.85, 1.0, 0.9, 0.75, 0.55, 0.30]
...
function generateSurfaces(
  ...
  if (isDark) {
    return {
      ...
      '--color-bg-blue-deep': oklchToHex({ l: 0.10, c: primaryChroma * 0.20, h: primaryHue }),
      '--color-bg-blue-dark': oklchToHex({ l: 0.15, c: primaryChroma * 0.25, h: primaryHue }),
      '--color-bg-blue-mid': oklchToHex({ l: 0.20, c: primaryChroma * 0.30, h: primaryHue }),
      '--color-bg-blue-light': oklchToHex({ l: 0.27, c: primaryChroma * 0.35, h: primaryHue }),
    }
  }

  // Light mode: primary-hue tinted surfaces
  const tint = primaryChroma * 0.40 // stronger tint in light mode
  return {
    '--color-background': oklchToHex({ l: 0.97, c: tint * 0.25, h: primaryHue }),
    '--color-surface': oklchToHex({ l: 1.0, c: 0, h: 0 }),
    '--color-surface-elevated': oklchToHex({ l: 0.95, c: tint * 0.50, h: primaryHue }),
    '--color-canvas-bg': oklchToHex({ l: 0.371, c: 0, h: 0 }),

    '--color-bg-100': oklchToHex({ l: 0.60, c: tint * 0.80, h: primaryHue }),
    '--color-bg-200': oklchToHex({ l: 0.81, c: tint * 1.50, h: primaryHue }),
    '--color-bg-300': oklchToHex({ l: 0.88, c: tint * 1.00, h: primaryHue }),
    '--color-bg-400': oklchToHex({ l: 0.92, c: tint * 0.65, h: primaryHue }),
    '--color-bg-light': oklchToHex({ l: 1.0, c: 0, h: 0 }),

    '--color-bg-blue-deep': oklchToHex({ l: 0.90, c: primaryChroma * 0.25, h: primaryHue }),
    '--color-bg-blue-dark': oklchToHex({ l: 0.85, c: primaryChroma * 0.35, h: primaryHue }),
    '--color-bg-blue-mid': oklchToHex({ l: 0.78, c: primaryChroma * 0.40, h: primaryHue }),
    '--color-bg-blue-light': oklchToHex({ l: 0.72, c: primaryChroma * 0.50, h: primaryHue }),
  }
}
...
function generateTextAndBorders(
  ...
  if (isDark) {
    return {
      ...
      '--color-text-dim': oklchToHex({ l: 0.50, c: primaryChroma * 0.12, h: primaryHue }),
  ...
  return {
    '--color-text': oklchToHex({ l: 0.30, c: tint * 0.60, h: primaryHue }),
    '--color-text-secondary': oklchToHex({ l: 0.46, c: tint * 0.70, h: primaryHue }),
    '--color-text-muted': oklchToHex({ l: 0.60, c: tint * 0.50, h: primaryHue }),
    '--color-text-dim': oklchToHex({ l: 0.50, c: tint * 0.60, h: primaryHue }),
    '--color-text-disabled': oklchToHex({ l: 0.75, c: tint * 0.30, h: primaryHue }),
...
/**
 * Generate a complete Silapse theme from two colors.
 *
...
export function generateTheme(input: ThemeInput): GeneratedTheme {
  ...
  for (const mode of ['dark', 'light'] as const) {
    ...
    vars['--color-header-bg'] = isDark
      ...
      : oklchToHex({ l: 0.91, c: p.c * 0.40, h: p.h })

    // Glow effects
    vars['--color-primary-glow'] = hexToRgba(
      oklchToHex({ l: isDark ? 0.80 : 0.60, c: p.c, h: p.h }),
      isDark ? 0.6 : 0.4,
    )
    vars['--color-accent-glow'] = hexToRgba(
      oklchToHex({ l: isDark ? 0.80 : 0.60, c: a.c, h: a.h }),
      isDark ? 0.6 : 0.4,
    )
    vars['--color-fusion-glow'] = hexToRgba(
      oklchToHex({ l: 0.70, c: (p.c + a.c) / 2, h: (p.h + a.h) / 2 }),
      isDark ? 0.7 : 0.5,
    )
    ...
    const danger = generateSemanticScale(25, 0.20, mode)   // red

    for (const [name, semantic] of [['success', success], ['warning', warning], ['danger', danger]] as const) {
    ...
    vars['--color-correct'] = oklchToHex({ l: isDark ? 0.80 : 0.50, c: 0.17, h: 145 })
    vars['--color-incorrect'] = oklchToHex({ l: isDark ? 0.65 : 0.50, c: 0.20, h: 25 })
