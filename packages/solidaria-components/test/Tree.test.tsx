  ...
  describe('rendering', () => {
    ...
    it('should wire horizontal droppable keyboard delegate methods in ltr and rtl', () => {
      ...
      try {
        ...
        expect(keyboardDelegate?.getKeyLeftOf?.('item-2')).toBe('item-1');
        expect(keyboardDelegate?.getKeyRightOf?.('item-1')).toBe('item-2');
        ...
        render(() => (
          <Tree
            items={createTestItems()}
            aria-label="RTL delegate tree"
            dragAndDropHooks={dragAndDropHooks as any}
          >
            {(item) => <TreeItem id={item.key}>{item.textValue}</TreeItem>}
          </Tree>
        ));
        expect(keyboardDelegate?.getKeyLeftOf?.('item-1')).toBe('item-2');
        expect(keyboardDelegate?.getKeyRightOf?.('item-2')).toBe('item-1');
      } finally {
        document.dir = originalDir;
      }
    });
