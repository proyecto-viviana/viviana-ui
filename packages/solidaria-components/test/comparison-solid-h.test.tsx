...
import { Calendar, CalendarButton, CalendarCell, CalendarGrid, CalendarHeading } from '../src/Calendar';
...
describe('comparison Solid h helper', () => {
  ...
  it('rejects zero-argument child functions at the helper boundary', () => {
    expect(() =>
      hc('div', {}, (() => []) as unknown as never),
    ).toThrow('Use child arrays, or renderProp(fn) for intentional render props.');
  });
  ...
  it('composes Button press handlers through hc child arrays', async () => {
    function Demo() {
      ...
      return hc(
        ...
        [
          hc(
            Button,
            { onClick: (_event: MouseEvent) => setPressCount((count) => count + 1) },
            ['Primary'],
          ),
        ],
      )();
    ...
    expect(screen.getByText('Primary').closest('[data-action-count]')).toHaveAttribute('data-action-count', '1');
  });
    ...
    function Demo() {
      ...
      return hc(
        ...
        [
          hc(
            TextField,
            { defaultValue: 'Quarterly report', onChange: setValue },
            [
      ...
      return hc(
        ...
        [
          hc(
            SelectTrigger,
            {},
            [hc(SelectValue, { placeholder: 'Pick one' })],
          ),
          hc(
            SelectListBox<TestItem>,
            {},
            renderProp((item: TestItem) =>
              hc(SelectOption, { id: item.id }, [item.label]),
            ),
          ),
        ],
      )();
  ...
  it('keeps RadioGroup selection reactive through hc composition', async () => {
    ...
    expect(comfortable.closest('[data-selected-key]')).toHaveAttribute('data-selected-key', 'comfortable');
  });
  ...
  it('keeps DialogTrigger open state and close render prop reactive through hc composition', async () => {
    ...
    const beforeCloseCount = Number(
      screen.getByRole('dialog').closest('[data-close-count]')?.getAttribute('data-close-count') ?? '0',
    );
  ...
  it('keeps Tooltip open state reactive through hc composition', async () => {
    function Demo() {
      ...
      return hc(
        ...
        [
          hc(
            TooltipTrigger,
            { delay: 0, onOpenChange: setIsOpen },
            [
              hc(Button, {}, ['Inspect']),
    ...
    expect(screen.getByRole('button', { name: 'Inspect' }).closest('[data-open]')).toHaveAttribute('data-open', 'true');
  });
  ...
  it('opens DatePicker calendar content through hc-composed date segment render props', async () => {
      ...
      return hc(
        ...
        [
          hc(
            ...
            {
              'aria-label': 'Due date',
              onChange: (nextValue: unknown) => setValue(nextValue == null ? '' : String(nextValue)),
            },
            [
              hc(DateInput, {}, renderProp((segment) => hc(DateSegment, { segment }))),
                ...
                [
                  hc(
                    ...
                    [
                      ...
                      hc(CalendarGrid, {}, renderProp((date) => hc(CalendarCell, { date }))),
                    ],
                  ),
                ],
              ),
    ...
    await waitFor(() => {
      expect(document.querySelector('.solidaria-DatePicker:not(.solidaria-DatePicker--placeholder)')).toBeInTheDocument();
    });
  ...
  it('keeps Toast visible count and dismissal reactive through hc composition', async () => {
    ...
    const [visibleCount, setVisibleCount] = createSignal(0);
      ...
      const showToast = () => {
        const key = addToast({ title: 'Saved successfully', description: 'Toast content', type: 'success' }, { timeout: 0 });
        shownKeys.push(key);
      };
    ...
    expect(screen.getByRole('button', { name: 'Show toast' }).closest('[data-visible-count]')).toHaveAttribute('data-visible-count', '1');
    ...
    expect(screen.getByRole('button', { name: 'Show toast' }).closest('[data-visible-count]')).toHaveAttribute('data-visible-count', '0');
  });
});
