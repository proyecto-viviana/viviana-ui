...
describe("Tabs (solid-spectrum)", () => {
  ...
  describe("underline variant (default)", () => {
    ...
    it("unselected tab gets border-transparent", () => {
      render(() => <TestTabs />);
      const tabs = screen.getAllByRole("tab");
      const unselectedTab = tabs.find((t) => t.getAttribute("aria-selected") !== "true");
  ...
  describe("disabled tab", () => {
    it("applies cursor-not-allowed", () => {
      ...
      const tabs = screen.getAllByRole("tab");
      const disabledTab = tabs.find((t) => t.textContent === "Second");
  ...
  describe("keyboard navigation", () => {
    it("arrow keys update roving tabindex", async () => {
      ...
      const selectedTab = tabs.find((t) => t.getAttribute("tabindex") === "0");
  ...
  describe("dynamic updates", () => {
    it("reselects first enabled tab when selected tab is removed", () => {
      ...
      render(() => (
        <Tabs items={items()} getKey={(item) => item.id} defaultSelectedKey="tab2">
          <TabList items={items()}>{(item) => <Tab id={item.id}>{item.label}</Tab>}</TabList>
          {items().map((item) => (
