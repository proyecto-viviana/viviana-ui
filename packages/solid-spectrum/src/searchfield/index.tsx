...
function SearchIcon(props: { class?: string }) {
  return (
    <svg class={props.class} viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="2">
...
export function SearchField(props: SearchFieldProps): JSX.Element {
  ...
  const inputClasses = (renderProps: SearchFieldInputRenderProps) => {
    ...
    return [base, sizeClass, paddingClass, variantClass, stateClass, hoverClass]
      .filter(Boolean)
      .join(" ");
  };
  ...
  const clearButtonClasses = (renderProps: SearchFieldClearButtonRenderProps) => {
    const base =
      "absolute flex items-center justify-center rounded-md transition-all duration-150 select-none";
      ...
      children={(renderProps: SearchFieldRenderProps) => (
        <>
          ...
          <Show when={local.errorMessage && renderProps.isInvalid}>
            <SearchFieldError class={errorClasses()}>{local.errorMessage}</SearchFieldError>
          </Show>
        </>
      )}
    />
