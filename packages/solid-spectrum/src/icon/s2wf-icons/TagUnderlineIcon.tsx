/*
 * Auto-generated from vendored React Spectrum S2 icon sources.
 * Do not edit by hand.
 */

import { type JSX } from "solid-js";
import { createIcon } from "../spectrum-icon";

function TagUnderlineIconSvg(props: JSX.SvgSVGAttributes<SVGSVGElement>): JSX.Element {
  const { class: className, ...rest } = props;
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="20"
      height="20"
      viewBox="0 0 20 20"
      {...rest}
      class={className}
    >
      <path
        d="M16.25,19H3.75c-.41406,0-.75-.33594-.75-.75s.33594-.75.75-.75h12.5c.41406,0,.75.33594.75.75s-.33594.75-.75.75Z"
        fill="var(--iconPrimary, #222)"
      />
      <path
        d="M10,16c-2.61914,0-4.75-2.13086-4.75-4.75V2.75c0-.41406.33594-.75.75-.75s.75.33594.75.75v8.5c0,1.79199,1.45801,3.25,3.25,3.25s3.25-1.45801,3.25-3.25V2.75c0-.41406.33594-.75.75-.75s.75.33594.75.75v8.5c0,2.61914-2.13086,4.75-4.75,4.75Z"
        fill="var(--iconPrimary, #222)"
      />
    </svg>
  );
}

export type TagUnderlineIconProps = JSX.SvgSVGAttributes<SVGSVGElement>;
export const TagUnderlineIcon = createIcon(TagUnderlineIconSvg);
export default TagUnderlineIcon;
