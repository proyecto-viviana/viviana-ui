...
export function TextArea(props: TextAreaProps): JSX.Element {
  ...
  const textareaClasses = (renderProps: TextFieldRenderProps) => {
    const base =
      "w-full rounded-md transition-all duration-200 outline-none resize-none overflow-hidden";
      ...
      children={(renderProps) => (
        <>
          ...
          <Show when={local.errorMessage && renderProps.isInvalid}>
            <TextAreaError class={errorClasses()}>{local.errorMessage}</TextAreaError>
          </Show>
        </>
      )}
    />
