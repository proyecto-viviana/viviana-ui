...
const variantStyles = {
  underline: {
    ...
    tab: {
      ...
      default:
        "border-transparent text-primary-400 hover:text-primary-200 hover:border-primary-400",
...
export function Tabs<T>(props: TabsProps<T>): JSX.Element {
  ...
  return (
    <TabsSizeContext.Provider value={{ size, variant }}>
      <HeadlessTabs {...headlessProps} class={getClassName} children={props.children} />
...
export function TabList<T>(props: TabListProps<T>): JSX.Element {
  ...
  const getClassName = (renderProps: TabListRenderProps): string => {
    ...
    return [base, orientationClass, sizeClass, variantClass, focusClass, customClass]
      .filter(Boolean)
      .join(" ");
  };
...
export function Tab(props: TabProps): JSX.Element {
  ...
  const getClassName = (renderProps: TabRenderProps): string => {
    ...
    return [variantBase, sizeClass, stateClass, focusClass, pressedClass, cursorClass, customClass]
      .filter(Boolean)
      .join(" ");
  };
...
export function TabPanels(props: TabPanelsProps): JSX.Element {
  ...
  return (
    <HeadlessTabPanels {...headlessProps} class={["flex-1", customClass].filter(Boolean).join(" ")}>
      {props.children}
    </HeadlessTabPanels>
  );
}
