...
export function ActionMenu<T>(props: ActionMenuProps<T>): JSX.Element {
  const [local, menuProps] = splitProps(props, [
    "label",
    "isQuiet",
    "isDisabled",
    "align",
    ...
    <HeadlessMenuTrigger>
      <HeadlessMenuButton
        ...
        class={(renderProps: MenuTriggerRenderProps) => {
          const base =
            "inline-flex items-center justify-center rounded-md transition-colors outline-none p-1.5";
          const quietClass =
            (local.isQuiet ?? true) ? "bg-transparent" : "bg-bg-400 border border-primary-600";
          ...
          return (
            <HeadlessMenuItem
              ...
              class={(renderProps: MenuItemRenderProps) => {
                const base =
                  "flex items-center cursor-pointer transition-colors duration-150 outline-none text-base py-2 px-4";
